...
export default function FollowersStats() {
  ...
  const totalFollowersStats = followers !== undefined && followers.reduce((acc, { vacation_id }) => {
  ...
  // type for stat
  const finalDiagramData = Object.entries(totalFollowersStats || {}).map(([id, count]) => {
    // Again, proceed only if vacations are NOT undefined
    const vacation = vacations !== undefined && vacations.find((v: Vacation) => v.vacation_id.toString() === id)
    return {
  ...
  const exportToCSV = () => {
    ...
    const csvContent = "Destination, Followers\n" + 
      finalDiagramData.map(row => `${row.destination}, ${row.totalFollowers}`).join("\n");
      ...
      <div className='w-full h-[400px]'>
        <ResponsiveContainer>
          <BarChart data={finalDiagramData}>
            ...
            <XAxis dataKey="destination" angle={-45} textAnchor="end" height={70} />
            <YAxis />
            <Tooltip />
