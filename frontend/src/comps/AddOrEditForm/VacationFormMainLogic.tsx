...
export default function AddVacationForm() {
  ...
  useEffect(() => {
    if (id && tempVacations) {
      console.log(
        "its edit mode so form is filled tempVacaion is",
        tempVacations,
      );
      // Convert Buffer to base64 string
      const base64String = btoa(
        String.fromCharCode.apply(null, tempVacations.image_path.data),
      );
      ...
      setStartDate(
        tempVacations.starting_date
          ? new Date(tempVacations.starting_date)
          : undefined,
      );
      setEndDate(
        tempVacations.ending_date
          ? new Date(tempVacations.ending_date)
          : undefined,
      );
      form.setFieldsValue({
