...
import { setRedisState, getRedisState } from "../config/redisConfig.js";
import { AuthRequest, Tokens, TokenPayload, UserRole } from "../types.js";
...
export const authToken = async (
  ...
  if (!accessToken) {
    req.authError = {
      status: 401,
      message: "there is no access token in local storage",
    };
  } else {
    jwt.verify(
      accessToken,
      process.env.JWT_SECRET as string,
      async (err: jwt.VerifyErrors | null, user: TokenPayload | undefined) => {
        if (err) {
          if (err.name === "TokenExpiredError") {
            ...
            if (!refreshToken) {
              req.authError = {
                status: 401,
                message: "there is no refresh token in local storage",
              };
            } else {
              try {
                ...
                req.authError = {
                  status: 500,
                  message: `unknown error with refreshAccessToken function ${err}`,
                };
              }
            }
