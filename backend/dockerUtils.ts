...
export async function fetchImageData(imagePath: string): Promise<string | Buffer> {
  const fullPath = path.join('/app', imagePath)
  try {
    return await fs.readFile(fullPath)
  } catch (err) {
    console.error('err in fetchImage data from docker func in dockerUtils file')
    throw err
  }
}
...
export async function handleFetchImageData(vacations: Vacation[]) {
  const updatedVacations = await Promise.allSettled(
    vacations.map(async (vacation: Vacation) => {
      try {
        ...
        console.error(`Failed to fetch image for vacation ${vacation.vacation_id}: ${err.message}`);
  ...
  return updatedVacations
    .filter((result): result is PromiseFulfilledResult<Vacation> => result.status === 'fulfilled')
